...
#import "GQBMenuListController.h"

@interface GQBMenuList_spaceCC : UICollectionViewCell
...
@implementation GQBMenuList_spaceCC

@end


@interface GQBMenuListController () <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate, YBLevelListViewDelegate>

@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) GQBMenuListView *titleView;
...
@implementation GQBMenuListController
...
- (void)initializeWithControllers:(NSArray<UIViewController *> *)controllers titleViewConfigModel:(GQBMenuListConfigModel *)configModel titleViewHeight:(CGFloat)titleViewHeight contentViewHeight:(CGFloat)contentViewHeight {
...
- (void)initializeWithControllers:(NSArray<UIViewController *> *)controllers titleViewConfigModel:(GQBMenuListConfigModel *)configModel titleViewFrame:(CGRect)titleViewFrame contentViewFrame:(CGRect)contentViewFrame {
...
- (void)traverseTitleViewAllSubView:(void (^)(GQBMenuListSubView * _Nonnull, NSInteger))block {
    [self.titleView traverseAllSubView:^(GQBMenuListSubView *subView, NSInteger idx) {
        block(subView, idx);
    }];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    GQBMenuList_spaceCC *cell = [collectionView dequeueReusableCellWithReuseIdentifier:[NSString stringWithFormat:@"YBLevelList_spaceCC%ld", indexPath.row] forIndexPath:indexPath];
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        for (NSInteger i = 0; i < self.childViewControllers.count; i++) {
            [self.collectionView registerClass:[GQBMenuList_spaceCC class] forCellWithReuseIdentifier:[NSString stringWithFormat:@"YBLevelList_spaceCC%ld", i]];
        }
    }
    return _collectionView;
}
- (GQBMenuListView *)titleView {
    if (!_titleView) {
        _titleView = [[GQBMenuListView alloc] initWithFrame:_titleViewFrame];
