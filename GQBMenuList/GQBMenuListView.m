...
@interface GQBMenuListView () <UIScrollViewDelegate>

//记录子视图数组
@property (nonatomic, strong) NSMutableArray<GQBMenuListSubView *> *subViewArr;
//记录子视图总共的长度
...
@property (nonatomic, strong) GQBMenuListConfigModel *configModel;


@end

@implementation GQBMenuListView
...
- (void)clickTapGesture:(UITapGestureRecognizer *)tapGesture {
    ...
    GQBMenuListSubView *subView = (GQBMenuListSubView *)tapGesture.view;
...
- (void)traverseAllSubView:(void (^)(GQBMenuListSubView *, NSInteger))block {
    [_subViewArr enumerateObjectsUsingBlock:^(GQBMenuListSubView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        block(obj, idx);
    }];
}
...
- (void)configUnderLineSelectedView {
    ...
    if (_subViewArr.count > _selectedIndex) {
        ...
        GQBMenuListSubView *selectSubView = _subViewArr[_selectedIndex];
...
- (void)configScrollViewContentsize {
    ...
    GQBMenuListSubView *finalSubView = _subViewArr.lastObject;
...
- (void)configAnimationOffsetX:(CGFloat)offsetX {
    ...
    // 左边按钮
    GQBMenuListSubView *leftSubView = _subViewArr[leftIndex];
    
    // 右边角标
    NSInteger rightIndex = leftIndex + 1;
    
    // 右边按钮
    GQBMenuListSubView *rightSubView;
...
//动态移动下划线
- (void)moveUnderLineSelectViewWithOffsetX:(CGFloat)offsetX leftSubView:(GQBMenuListSubView *)leftSubView rightSubView:(GQBMenuListSubView *)rightSubView {
    // 获取两个标题中心点距离
...
//动态改变字体渐变
- (void)configTitleColorGradientWithOffset:(CGFloat)offsetX rightSubView:(GQBMenuListSubView *)rightSubView leftSubView:(GQBMenuListSubView *)leftSubView
{
...
- (void)configSubView {
    ...
    [_configModel.titleArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        //创建subView (复用之前创建的内存)
        GQBMenuListSubView *subView;
        if (_subViewArr.count > idx) {
            subView = _subViewArr[idx];
        } else {
            subView = [GQBMenuListSubView new];
        }
        ...
        GQBMenuListSubView *lastSubView;
...
- (void)configSubViewTitleOfUnSelected:(GQBMenuListSubView *)subView width:(CGFloat *)width {
...
- (void)configSubViewTitleOfSelected:(GQBMenuListSubView *)subView width:(CGFloat *)width {
...
//返回值为  是否是富文本 YES 为是
- (BOOL)configSubViewTitleWithSubView:(GQBMenuListSubView *)subView infoArr:(NSArray<id> *)infoArr width:(CGFloat *)width {
...
- (BOOL)configSubViewTitleImageOfSelected:(GQBMenuListSubView *)subView width:(CGFloat *)width {
    return [self configSubViewTitleImageWithSubView:subView infoArr:_configModel.imageInfoSelectedArr width:width];
}
- (BOOL)configSubViewTitleImageOfUnSelected:(GQBMenuListSubView *)subView width:(CGFloat *)width {
    return [self configSubViewTitleImageWithSubView:subView infoArr:_configModel.imageInfoArr width:width];
}

- (BOOL)configSubViewTitleImageWithSubView:(GQBMenuListSubView *)subView infoArr:(NSArray<id> *)infoArr width:(CGFloat *)width {
...
- (BOOL)configImage:(GQBMenuListSubView *)subView tempObj:(id)tempObj {
    if (!tempObj) {
        return NO;
...
- (void)scrollSelectedViewToCenter {
    ...
    GQBMenuListSubView *subView = _subViewArr[_selectedIndex];
